...
public sealed class Discolorator : MonoBehaviour
{
    [Space]
    ...
    public void Breathing(Color targetColor, float targetIntensity, float lapTime, float inhaleDelayMin, float inhaleDelayMax, float exhaleDelayMin, float exhaleDelayMax)
    {
        StopBreathing();

        breathingCoroutine = _Breathing(targetColor, targetIntensity, lapTime, inhaleDelayMin, inhaleDelayMax, exhaleDelayMin, exhaleDelayMax);

        StartCoroutine(breathingCoroutine);
    }

    public void Breathing(Color targetColor, float lapTime, float inhaleDelayMin, float inhaleDelayMax, float exhaleDelayMin, float exhaleDelayMax)
    {
        Breathing(targetColor, _intensity, lapTime, inhaleDelayMin, inhaleDelayMax, exhaleDelayMin, exhaleDelayMax);
    }

    public void Breathing(Color targetColor, float breathingtIntensity, float lapTime)
    {
        Breathing(targetColor, breathingtIntensity, lapTime, lapTime, lapTime, lapTime, lapTime);
    }

    public void Breathing(float breathingtIntensity, float lapTime, float inhaleDelayMin, float inhaleDelayMax, float exhaleDelayMin, float exhaleDelayMax)
    {
        Breathing(_color, breathingtIntensity, lapTime, inhaleDelayMin, inhaleDelayMax, exhaleDelayMin, exhaleDelayMax);
    }

    public void Breathing(float breathingtIntensity, float lapTime)
    {
        Breathing(_color, breathingtIntensity, lapTime, lapTime, lapTime, lapTime, lapTime);
    }

    private IEnumerator _Breathing(Color targetColor, float targetIntensity, float lapTime, float inhaleDelayMin, float inhaleDelayMax, float exhaleDelayMin, float exhaleDelayMax)
    {
