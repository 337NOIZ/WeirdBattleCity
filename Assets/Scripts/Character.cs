...
public abstract class Character : MonoBehaviour
{
    ...
    protected virtual void _Awaken()
    {
    ...
    {
        var _itemInfo = FindItem(itemInfo.itemType, itemInfo.itemCode);

        if (_itemInfo != null)
        {
            int stackCount_Max = _itemInfo.stackCount_Max;

            if (_itemInfo.stackCount < stackCount_Max)
            {
                _itemInfo.stackCount += itemInfo.stackCount;

                if (_itemInfo.stackCount > stackCount_Max)
                {
                    _itemInfo.stackCount = stackCount_Max;
                }
