...
public sealed class PlayerMovement : MonoBehaviour
{
    ...
    private void Move()
    {
        ...
        {
            player.animator.SetFloat("movingMotionSpeed", 0f);
        }
        ...
        else
        {
            ...
            player.animator.SetFloat("movingMotionSpeed", movementInfo.movingSpeed_Multiply);
            ...
            player.GetComponent<Rigidbody>().MovePosition(player.GetComponent<Rigidbody>().position + movePosition * movingSpeed * movementInfo.movingSpeed_Multiply * Time.deltaTime);
        }
    ...
    public void Jump()
    {
        if (movementInfo.jumpCount < movementData.jumpCount_Max + movementInfo.jumpCount_Max_Extra)
        {
            ++movementInfo.jumpCount;

            player.GetComponent<Rigidbody>().velocity = Vector3.zero;

            player.GetComponent<Rigidbody>().AddForce(new Vector3(0f, movementData.jumpForce * movementInfo.jumpForce_Multiply, 0f), ForceMode.Impulse);

            player.animator.SetBool("isGrounded", false);

            player.animator.SetTrigger("jumpingMotion");
        }
    }
}